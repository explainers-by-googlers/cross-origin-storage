...
1. `WritableStream` `final` → non-final (one-line), building `FileSystemWritableFileStream` via
   ...
   already used for transferable streams) rather than needing new streams-machinery — this
   turned out to have solid prior art in this codebase, contrary to the initial assumption that
   it would be the single biggest risk (see Risks below).
...
- `validate_cos_hash(...)` — validate `algorithm` against the WebCrypto-recognized name set
  ...
  — bake this in from day one; Servo's own notes flag getting this wrong (validating only
  SHA-256's shape) as an exploitable path-traversal bug once the hash `value` is used to build
  an on-disk path in Phase 5.
...
**Security-critical ordering** (Servo got this wrong initially, per the engineering notes): each
chunk write must call `temp_file->write_until_depleted(data)` (or seek+write for
seek/truncate command types) **first**, and only feed the hasher **after** that succeeds — never
update the running hash/byte-count before the fallible disk write is confirmed. `did_finish_cos_write`
