...
- **If you're building `FileSystemWritableFileStream` from scratch for COS**, with no OPFS to lean on, you don't have to default to a memory-backed buffer just because there's nothing to inherit from. Going straight to a real disk-backed temp file (`seek()`/`write()`/`truncate()` all operate on it directly via ordinary file I/O) sidesteps the entire unbounded-allocation-DoS category in [§10](#sec-10) at the design level, rather than needing a runtime cap on it.
  - **Servo:** built from scratch with an in-memory byte vector as the buffer (`seek()`/`write()`/`truncate()` all mutate it directly, sitting in script-process memory until `close()`), specifically because there was no pre-existing OPFS to build on. The "hold gigabytes in RAM" cost that follows is a consequence of that choice, not something you should assume you'll inherit too.
  - **Ladybird:** also built from scratch, but chose the disk-backed temp-file option instead. Not free — it caused one bug (a temp file opened write-only for streaming needs to be re-opened, or seeked back and read, once `close()` needs to hash the final content) — but a small, one-time cost against avoiding a whole class of bug.
...
**Gate 1 — the Public Hash List (PHL).** This is the load-bearing privacy mechanism of the whole feature. Without it, `'*'` disclosure would be a trivial cross-site tracking/probing oracle: site A writes `sha256(some-value-only-known-if-you-visited-site-B)`, wildcard-scoped; site A later probes for that exact hash from any other context and learns whether the user visited site B. The PHL closes this by only ever disclosing a wildcard-scoped entry if its hash is **independently confirmed present on a curated, cross-vendor, rolling-release allowlist** of "ubiquitous, corroborated" resource digests — modeled directly on the Public Suffix List's own governance precedent (per the spec/explainer's framing: cross-vendor governance, not any single browser's list). A hash not on the list **fails closed**: it's simply not disclosed, indistinguishable from a genuine miss. The PHL only lists SHA-256 digests, so a wildcard-scoped entry hashed with any other recognized algorithm (SHA-1/384/512) can never pass this gate — check the algorithm before even attempting the lookup.
  ...
  **Servo:** builds and consumes a real PHL pipeline:
  ...
  - **Verify a checksum after downloading it.** The PHL repo publishes a companion `<file>.sha256` (standard `sha256sum`-compatible format) right next to the data file. Fetch and check it before trusting the download and compiling it into your browser — this guards against a corrupted/truncated transfer or a compromised LFS response landing in something that ships to every user.
  - The raw `.dat` format is line-oriented: `//`-prefixed comment lines (provenance annotations, safe to ignore) and bare lowercase-hex SHA-256 digest lines. Servo re-encodes this at build time into **sorted, packed 32-byte binary digests with no delimiters**, purely to keep the compiled-in resource compact and to enable `binary_search` (O(log n)) at lookup time instead of needing a `HashSet` (a `HashSet<[u8; 32]>` of hundreds of thousands of entries costs meaningfully more memory for the same guarantee, and the list is read-only after load, so a sorted array is strictly better here).
  - **Refresh it periodically, automatically, in CI** — it's an explicitly rolling-release list, not a one-time snapshot; the spec's governance model depends on it being kept current. Servo refreshes weekly.

  **Ladybird:** ships a seed-only placeholder PHL rather than the full fetch/verify/refresh pipeline above — a deliberately deferred piece, not a design alternative. If you're implementing this for real, Servo's pipeline above is the fuller reference.
...
**Ladybird:** fixed this with a separate, cheap, read-only bound, distinct from the real budget check: computed once at write-session start (so it's not a syscall per chunk) from the same budget fraction the real check uses, then compared against the temp file's actual size after every `write()`/`truncate()` (a `truncate()` sets the tracked size directly; a `write()` only ever grows it up to the resulting stream position). A breach fails the session the same way a disk I/O failure already does ([§3](#sec-3)), reusing the existing cleanup path rather than inventing a new one. This specific gap applies most directly to a disk-backed writable stream; a memory-backed one is already covered by whatever cap you put on `seek()`/`truncate()`-triggered allocation ([§10](#sec-10)).
...
2. **Unbounded-allocation denial-of-service via `seek()`/`truncate()`, if you buffer in memory.** Because random-access edits are allowed before `close()` ([§3](#sec-3)), a page can call `truncate(hugeNumber)` — or `seek(hugeNumber)` followed by any `write()` — and have a memory-backed writable-stream sink attempt to allocate and zero-fill an arbitrarily large buffer *immediately*, synchronously, from a single script call, long before your registry's own storage-budget check ever gets a chance to reject anything at `close()` time. This is specifically a risk for a memory-backed writable stream ([§3](#sec-3)); a genuinely disk-backed implementation trades this for an analogous but different resource-exhaustion risk (a huge `truncate()` claiming disk space instead of RAM — see [§6](#sec-6)'s streaming-time quota gap), which still needs its own guard, just via a different mechanism. Either way: **cap whatever resource a single `seek()`/`truncate()` call can claim, and check that cap *before* attempting the resize/allocation, not after.** **Servo:** uses a 4 GiB in-memory cap, chosen well above any real individual shard size its other budget constants are sized around, while still bounding worst-case allocation. **Ladybird:** sidesteps this category entirely by being disk-backed from the start (see [§3](#sec-3)), relying on the streaming-time size cap in [§6](#sec-6) instead of an in-memory allocation cap.
...
| Max in-memory write buffer *(Servo only — memory-backed stream; see [§3](#sec-3))* | 4 GiB | Caps `seek()`/`truncate()`-triggered allocation, checked before resize |
| Streaming-time size cap *(Ladybird only — disk-backed stream; see [§6](#sec-6))* | Same fraction as the real per-origin budget | Cheap, session-cached bound checked on every `write()`/`truncate()` during streaming, distinct from the authoritative check at `close()` |
| PHL refresh cadence *(Servo only; Ladybird's PHL is a seed-only placeholder — see [§4](#sec-4))* | Weekly | Keeps the rolling-release allowlist current |
