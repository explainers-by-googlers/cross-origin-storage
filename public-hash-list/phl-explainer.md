...
An early implementation of the Public Hash List proposal is maintained at [https://github.com/tomayac/public-hash-list/](https://github.com/tomayac/public-hash-list/). To keep the list evergreen, it automatically ingests "top" resources from nine CDN sources (Google Hosted Libraries, Microsoft Ajax CDN, cdnjs, jsDelivr, npm popular packages, Chromium pervasive resources, the YouTube player, Google Maps JS API, and Google Fonts) plus a hand-curated AI model-hub section drawn from the top-100 most-downloaded models on the Hugging Face Hub. Like the PSL, we can consider supporting [manual additions](#manual-additions) as well, however such a process has not yet been designed. The canonical [Public Hash List](https://raw.githubusercontent.com/tomayac/public-hash-list/refs/heads/main/data/public-hash-list.dat) flat file and [its SHA-256 integrity file](https://raw.githubusercontent.com/tomayac/public-hash-list/refs/heads/main/data/public-hash-list.dat.sha256) are published in the [data/](https://github.com/tomayac/public-hash-list/tree/main/data) directory of that repository.
...
The corpus is small and slow-growing, which is the whole point: ubiquity is rare. The current candidate set is **3,792 distinct hashes** drawn from **19 host origins**, with the long tail dominated by version proliferation (dozens of historical jQuery builds, hundreds of versioned YouTube `player_ias` files) rather than distinct libraries. Storage is trivial: a SHA-256 digest is 32 bytes, so even 100,000 entries is \~3.2 MB of raw digests, and a probabilistic membership filter shrinks the hot-path structure to well under a megabyte at a tunable false-positive rate. Lookups are local set-membership tests with no network dependency; query volume is bounded by `requestFileHandle()` calls. The dominant growth driver is new *versions* of already-listed resources, so the list grows roughly with release cadence across the curated sources, not with the size of the Web.
